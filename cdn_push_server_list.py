...
def parser_command():
    usage = '''%prog     
       ...
       -v    版本v1.1.5
       eg:   %prog -p qq -o android -v v1.1.5  同步qq选服列表知道cdn v1.1.5目录'''
    ...
    parser.add_option("-v","--version",action="store",type="str",dest="version",help="指定版本: v1.1.5",metavar="版本: v1.1.5")
...
def main(args):
    try:
        ...
        if option.platform == "tszn":
            file_list_url = "%s?os=%s&test=false&other=%s"%(file_list,option.os,option.platform)
        else:
            file_list_url = "%s?os=%s&test=false&other="%(file_list,option.os)
        #下载server_list
        #print file_list_url
        ...
        Rsync_file(passfile,cdn_user,cdn_dir,cdn_server,local_file_path,sub_dir)
